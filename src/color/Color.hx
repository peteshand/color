...
abstract Color(Null<UInt>) from Int from UInt to Null<UInt> {
	...
	static public function random(alpha:Null<Int> = null, red:Null<Int> = null, green:Null<Int> = null, blue:Null<Int> = null):Color {
		...
		if (alpha == null)
			randomColor.alpha = Math.round(0xFF * Math.random());
		else
			randomColor.alpha = alpha;

		if (red == null)
			randomColor.red = Math.round(0xFF * Math.random());
		else
			randomColor.red = red;

		if (green == null)
			randomColor.green = Math.round(0xFF * Math.random());
		else
			randomColor.green = green;

		if (blue == null)
			randomColor.blue = Math.round(0xFF * Math.random());
		else
